...
function draw() {
  ...
  if (mouseIsPressed) {
    for (let i = 0; i < 5; i += 1) p.push(new P(mouseX + random(-8, 8), mouseY + random(-8, 8)));
  }
  if (frameCount % 2 === 0) p.push(new P(width / 2 + random(-20, 20), height - 12));
...
function sample(x, y) {
  let i = (constrain(floor(x / width * g.width), 0, g.width - 1) + constrain(floor(y / height * g.height), 0, g.height - 1) * g.width) * 4;
...
class P {
  ...
  show() {
    fill((this.h + sample(this.p.x, this.p.y) * 120) % 255, 200, 255, this.l);
